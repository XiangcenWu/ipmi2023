...
f_select.load_state_dict(torch.load("/raid/candi/xiangcen/meta_data_select/ipmi2023/ipmi2023/F_SEL_not_cls.pt", map_location=device))
f_select.to(device)
...
for batch in selection_loader:
    ...
    with torch.no_grad():
        selection = torch.argmax(f_select(img).sum(dim=0).flatten(0))
        pred = f_seg(img)
...
print(mmd(overall[torch.randint(0, 12, (1, ))].view(-1, 3), overall, 3.))