...
f_seg.load_state_dict(torch.load("/raid/candi/xiangcen/meta_data_select/ipmi2023/f_seg_v1.pt", map_location='cpu'))
f_seg.eval()


device= 'cuda:2'
feature_extractor = copy.deepcopy(f_seg.swinViT).to(device).eval()

f_seg.to(device).eval()


f_select = SelectionNet(num_sequence, 3072)
f_select.load_state_dict(torch.load("/raid/candi/xiangcen/meta_data_select/ipmi2023/F_SEL_pretrain_vit.pt", map_location=device))
f_select.to(device).eval()
...
for batch in selection_loader:
    ...
    features_e = feature_extractor(img)[-1].flatten(1)

    with torch.no_grad():
        selection = f_select(features_e).view(5, )

        selection = torch.argmax(selection)
        pred = f_seg(img)
...
print(     torch.abs(sel.mean(0) - overall.mean(0))            )
...
for i in range(5):
    ...
    print(torch.abs(rand_selection - overall_mean  ))
